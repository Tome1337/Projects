        ...
        &.error
         {
            ...
            &:after { content: var(--form-input-error-icon, var(--form-error-icon, "\f18e")); color: var( --form-input-error-color, var(--form-error-color, #ed1c24 )); }
        }
        ...
        &.success
         {
            ...
            &:after{ content: var(--form-input-success-icon, var(--form-success-icon, "\f18c")); color: var( --form-input-success-color, var(--form-success-color, #1bb934 )); }
        }
    }
</style>
<script>
...
<label class={'Input ' + ( _value.toString().length > 0 ? '' : ' empty' ) + _error + _success + _disabled + ( _label ? ' label' : '' ) + _className } style={_style + ( !_label ? 'padding-top: 0;' : '' )} >
    <input name={_name} id={_id} type={_type} value={_value} onfocus={"input.Focus(this.parentNode)"+_onFocus} onblur={"input.Blur(this.parentNode);"+_onBlur} data-pattern={_pattern} onchange={_onChange} onkeyup="window.input.keyUp(this, event)" autocomplete={_autocomplete} required={_required} />
    if( _label )
    {
