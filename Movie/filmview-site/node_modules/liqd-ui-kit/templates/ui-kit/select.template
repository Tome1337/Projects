        ...
        window.Select = class Select
        {
            ...
            keyup( element )
            {
                if( element.getAttribute('data-request') )
                {
                    ...
                    {
                        DOM_request( 'POST', url, model+"( name LIKE '%"+element.value+"%' )[ value: id, label : name ]", ( data ) =>
                        {
                            ...
                            {
                                div = wrapper.querySelector('[data-value="'+item.value+'"]');
                                if( !div )
                                {
                                    isSelected = ( typeof this._values.find( f => { return ( f.value === item.value ); } ) !== 'undefined' );
                                    div = document.createElement('div'); div.className = 'select-'+( typeof item.label === 'string' ? 'option' : 'group' )+( isSelected ? ' active' : '' ); div.setAttribute('data-value', item.value);
                                    div.innerHTML = ( typeof item.label === 'string' ? item.label : item.value );
                                    if( typeof item.label === 'string' ){ div.addEventListener( 'click', this.select.bind(this, div) ); }
                                }
                                ...
                                if( typeof data[key] === 'object' )
                                {
                                    ...
                                    {
                                        console.log(s_key)
                                        sub_div = wrapper.querySelector('[data-value="'+data[key][s_key].id+'"]');
                                        if( !sub_div )
                                        {
                                            isSelected = ( typeof this._values.find( f => { return ( f.value === data[key][s_key].id ); } ) !== 'undefined' );
                                            sub_div = document.createElement('div'); sub_div.className = 'select-option'+( isSelected ? ' active' : '' ); sub_div.setAttribute('data-value', data[key][s_key].id);
                                            sub_div.innerHTML = data[key][s_key].value;
...
let _model = ( typeof model !== 'undefined' ? model : null );
...
<div style={ _style + ( !_label ? 'padding-top: 0;' : '' ) } class={'Select' + _fill + _selected +_class + _disabled + _error + _success  + ( _label ? ' label' : '' ) }>
<label for={_id} class="label">{( _label ? _label : '' )}</label>
...
<div class="select-wrap">
    ...
    {
    <input placeholder="Search" autocomplete="off" class="input-element-text" onkeypress="return event.keyCode!=13" type="text" data-request={_request} data-model={_model}/>
